...
function AdminHrReflectionPublishControls({
  matchId,
  sessionNumber,
  viewerRole,
  clientSubmitted,
  ...
  return (
    <section className="rounded-2xl border border-amber-200 bg-amber-50/60 px-5 py-5">
      ...
      <p className="mt-2 text-sm leading-relaxed text-amber-950/90">
        クライアントの「良かったところ / もっと良くなるところ / 満足度 / 理由」だけを、企業人事の1on1一覧から閲覧可能にします。スコアやパートナー評価は含めません。
      </p>
      {loading ? (
        ...
        <div className="mt-3 space-y-3">
          <p className="text-sm text-slate-800">
            ...
            <span className="font-semibold">
              {published ? `公開中${publishedAt ? `（${new Date(publishedAt).toLocaleString("ja-JP")}）` : ""}` : "未公開"}
            </span>
          </p>
    ...
    return (
      <div className="space-y-8">
        ...
        <AdminHrReflectionPublishControls
          ...
          viewerRole={viewerRole}
          clientSubmitted={roundStatus.clientSubmitted}
        />
